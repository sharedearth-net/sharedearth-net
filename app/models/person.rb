  ...
  def  public_activities(current_user)
    if current_user.person.id != self.id
      ...
      activites = ActivityLog.find(:all, 
                         ...
                         self.id, self.class.to_s, current_user.person.id, current_user.person.class.to_s,  EventType.current_actions_underway], :order => 'created_at DESC').take(7)
    else
      current_user.person.activity_logs.order("#{ActivityLog.table_name}.created_at DESC").take(7)
    end
  end
