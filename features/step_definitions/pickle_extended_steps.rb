Given /^"(.*)" is the owner of item with name "(.*)"$/ do |name, item|
    ...
    Factory(:item, :owner => person, :name => item, :item_type => item)
end
...
Given /^I am the owner of shareage item with name "([^"]*)"$/ do |item|
    person = Person.find_by_name("John")
    Factory(:item, :owner => person, :name => item, :item_type => item, :purpose => Item::PURPOSE_SHAREAGE)
end
...
Given /^"(.*)" has trusted relationship with "(.*)"$/ do |person1, person2|
    person1 = Person.find_by_name("#{person1}")
    person2 = Person.find_by_name("#{person2}")
    Factory(:human_network, :entity_id => person1.id, :entity_type => "Person", :person_id => person2.id)
    Factory(:human_network, :entity_id => person2.id, :entity_type => "Person", :person_id => person1.id)
end
